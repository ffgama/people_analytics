...
roc = round(roc_auc_score(data_test['Attrition'], data_test['prediction']),2)
recall = round(recall_score(data_test['Attrition'], data_test['prediction']),2)
precision = round(precision_score(data_test['Attrition'], data_test['prediction']),2)
...
# Confusion Matrix
cm = confusion_matrix(data_test['Attrition'], data_test['prediction'], labels=[0,1])
cm
...
# TP -> modelo previu que o colaborador se desligaria e ele de fato se desligou (43)
# TN -> modelo previu corretamente que o colaborador nao se desligaria e ele de fato nao se desligou (307)

# Erros do nosso modelo
# FP -> modelo previu que o colaborador se desligaria mas ele nao se desligou (73)
# FN -> modelo previu que o colaborador nao se desligaria mas na verdade ele se desligou (18)

df_res = DataFrame(
    classification_report(data_test['Attrition'], data_test['prediction'], output_dict=True)).T
...
# Aumento dos falsos positivos: aumento da probabilidade do modelo da certeza do modelo
# de que o colaborador ira se desligar embora uma boa parte deles nao se desligue.

# O efeito de um recall muito alto tem como consequencia de que o RH ao observar uma alta
# probabilidade de attrition, tera que propor acoes para grande parte de colaboradores
...
query_template_not_attrition = data_test.query('Attrition==0')['prob_desligamento'].values
query_template_attrition = data_test.query('Attrition==1')['prob_desligamento'].values
...
# Ainda assim, nesse momento estamos em um step de validacao junto ao usuario de negocio.
# Poderiamos decidir usar o modelo (ainda que em um ambiente de teste) para validacao.
# Que crenca temos?
...
# E evidente de que um desligamento do colaborador tende a trazer como consequencia
# outros problemas para a empresa tais como: gerenciar o clima do time, 
# descontinuacao de trabalho, abertura de novo processo seletivo e assim por diante.

# Mas tambem uma forma de olharmos para o problema seria mensurando o impacto
# financeiro.

# Ambiente ficticio
# Sentamos com o RH e financeiro e identificamos que os custos de desligamento de um colaborador para a empresa 
...
# custos com desligamentos dos 63 colaboradores 
# 3,8 milhoes
...
# probabilidade acima de 50% de attrition entao poderiamos economizar 
# ate 2,2 milhoes!
